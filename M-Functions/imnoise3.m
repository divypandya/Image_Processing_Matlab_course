...
% [r, R, SI = IMNOISE3(M, N, C, A, B), generates a spatial 
...
% frequency rectangle center at [floor(M/2) + 1, floor(N/2) + 11, 
...
for j = 1:K 
    ...
    % of R(u, v) associated with a sinusoid is 0 unless u = -uO and 
    % v = -vO:
    u1 = floor(M/2) + 1 - C(j, 1); 
    v1 = floor(N/2) + 1 - C(j, 2); 
    R(u1, v1) = 1i*M*N*(A(j) /2) * exp(-1i*2*pi*(C(j, 1)*B(j, 1)/M... 
        + C(j, 2)*B(j, 2) /N)); 
    % Conjugate. The second term is zero unless u = uO and v = vo: 
