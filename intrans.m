...
% 
% G INTRANS(F, 'log', C, CLASS) computes C*10g(1 + F) and 
...
% Verify the correct number of inputs. error(nargchk(2, 4, nargin)) 
if strcmp(method, 'log')
    % The log transform handles image classes differently than the 
    % other transforms, so let the 10gTransform function handle that 
    % and then return. 
