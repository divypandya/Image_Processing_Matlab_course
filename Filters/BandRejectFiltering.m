...
%Bandreject filtering using bandfilter() function
DO=0.05*PQ(2);
H=bandfilter('gaussian','reject',PQ(1),PQ(2),DO,20);
F=fft2(f,PQ(1),PQ(2));
g=dftfilt(f,H);
g=revertclass(g);
figure,imshow(fftshift(H));
title('Frequency Spectra of Band-Reject Filter');
...
title('Mesh grid view of Band-Reject filter');
axis tight;
colormap([0 0 0]);
axis off;