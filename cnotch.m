...
% coordinates in MAT LAB run from 1 to M and 1 to N). Coordinates 
...
% 'btw'        Butterworth notchpass filter of order n. 
...
for I = 1:K 
    ...
    filt = fftshift(hpfilter(type, Usize, Vsize, DO(1), n)); 
    % Insert FILT in H. 
...
if delu >= 0 && delv >= 0 
    filtCommon = filt(1:M, 1 :N); % Displacement is in 01. 
elseif delu < 0 && delv >= 0 
    filtCommon = filt(U + 1:U + M, 1:N); % Displacement is in 02. 
elseif delu < 0 && delv < 0 
    filtCommon = filt(U + 1:U + M, V + 1:V + N); % 03 
elseif delu >= 0 && delv <= 0 
    filtCommon = filt(1:M, V + 1:V + N); % 04 
end 
% Compute the product of Hand filtCommon. They are registered. 
...
P = P.*(rot90(P,2)); 
P = H.*P; 
% A new notch and its conjugate were inserted. 
%- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - --% 
...
% Insert the new rowand/or column if appropriate. 
