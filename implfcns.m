...
% output membership functions. The number of elements of OUTMF can % either be numel(L) or numel(L)+1. If numel(OUTMF) is numel(L)+1, % then the 'extra' membership function is applied to an % automatically computed 'else rule.' (See Section 3.6.3.). The % inputs Z1, Z2, etc., can all be scalars, or they can all be % vectors of the same size (i.e., these vectors would contain % multiple values for each of the inputs). % % Q is a 1-by-numel(OUTMF) cell array of implication function % handles. % % Call the i-th implication function on an input V using the % syntax: % % Q{i}(V) 
Z = varargin;
